import { ShengmuList } from '../../Pinyin'

import quanPinSchema, { type as quanpinType } from './Quanpin'

import type { Quanpin, Shuangpin, ZeroShengmu } from '../../Pinyin'
...
export default class PinyinSchemaRegister {
  ...
  public getPinyin(schemaType: string, quanpin: string) {
    ...
    if (shengmu) {
      return [
        schema.map[shengmu as Shuangpin],
        schema.map[quanpin.replace(shengmu, '') as Shuangpin],
      ].join('')
    }
